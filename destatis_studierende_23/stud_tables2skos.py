...
g = Graph()
DESTATIS_STUDIERENDE = Namespace(BASE_URL + "/")
ISCED = Namespace("https://publications.europa.eu/resource/authority/snb/isced-f/")
g.bind("skos", SKOS)
g.bind("dcterms", DCTERMS)
g.bind("destatisstudierende", DESTATIS_STUDIERENDE)
g.bind("isced", ISCED)


# ConceptScheme erzeugen
g.add((DESTATIS_STUDIERENDE.scheme, RDF.type, SKOS.ConceptScheme))
g.add(
    (
        DESTATIS_STUDIERENDE.scheme,
        DCTERMS.title,
        Literal(
...
g.add((DESTATIS_STUDIERENDE.scheme, DCTERMS.creator, Literal("Statistisches Bundesamt", lang="de")))
g.add((DESTATIS_STUDIERENDE.scheme, DCTERMS.created, Literal("2024-01-11", datatype=XSD.date)))
g.add((DESTATIS_STUDIERENDE.scheme, DCTERMS.license, Literal("© Statistisches Bundesamt (Destatis), 2025. Vervielfältigung und Verbreitung, auch auszugsweise, mit Quellenagabe gestatted.", lang="de")))

# Fächergruppen (FG)
for _, row in fg_df.iterrows():
    fg_id = row["id"]
    fg_label = row["label"]
    fg_uri = URIRef(f"{DESTATIS_STUDIERENDE}{fg_id}")
    ...
    g.add((fg_uri, SKOS.topConceptOf, DESTATIS_STUDIERENDE.scheme))
    g.add((DESTATIS_STUDIERENDE.scheme, SKOS.hasTopConcept, fg_uri))
    g.add((fg_uri, SKOS.inScheme, DESTATIS_STUDIERENDE.scheme))
...
for _, row in stb_df.iterrows():
    ...
    luf_label = row["label"]
    fg_id = row["parent_id"]
    luf_uri = URIRef(f"{DESTATIS_STUDIERENDE}{fg_id}.{luf_id}")
    fg_uri = URIRef(f"{DESTATIS_STUDIERENDE}{fg_id}")
    ...
    g.add((luf_uri, SKOS.inScheme, DESTATIS_STUDIERENDE.scheme))
...
for _, row in stf_df.iterrows():
    ...
    fgb_uri = URIRef(f"{DESTATIS_STUDIERENDE}{fg_id}.{luf_id}.{fgb_id}")
    luf_uri = URIRef(f"{DESTATIS_STUDIERENDE}{fg_id}.{luf_id}")
    ...
    g.add((fgb_uri, SKOS.inScheme, DESTATIS_STUDIERENDE.scheme))
...
source_graph = Graph()
source_graph.parse("destatis_studierende_23/hochschulfaechersystematik.ttl", format="turtle")

# Load missing translations from CSV
missing_df = pd.read_csv("destatis_studierende_23/missing_translations.csv", dtype=str)
...
# Serialize result
g.serialize("destatis_studierende_23/destatis_studierende_skos.ttl", format="turtle")
